...
Nomad is the server, Houdini is a client. Every node in the session shares one
connection.

## Install

The assets are prebuilt — nothing to compile.
...
That one file puts both the assets and the Python on Houdini's path; it copies
nothing into your Houdini preferences.

## Use

In Nomad, open the **Link** menu to start the server. Connect from Houdini and
approve the request the first time. The bridge saves the pair token to
...
| vertex color / opacity | point `Cd` / `Alpha` |
...
The In SOP skips hidden objects, so Nomad's visibility toggle reaches
Houdini.
...
back. The Out SOP sends n-gons as `face_format: "corners"` when Nomad advertises
`ngon`, and fans them into triangles here otherwise.
...
landed in the wrong version folder. Houdini's *Windows → Shell* shows what it
loaded: `hou.hda.loadedFiles()` lists `nomad_link.hda`.

**Connect finds nothing.** Discovery needs both devices on the same network, and
an iPad does not answer a UDP broadcast when it is asleep. Open Nomad's **Link**
menu first, then type the device's address into **Host** if discovery still
fails — that menu shows the address.
...
never require it, because `otls/nomad_link.hda` is committed and Houdini loads
the Python live from `houdini/python/nomad_link`.

```
hython houdini/build_hda.py
```

`build_hda.py` defines both nodes: their parameters, the wrangles inside
*Nomad Link Out*, and the Python SOPs that call into the package. It rewrites
`otls/nomad_link.hda` in place.
...
- `test_houdini.py` — the real assets in Houdini: both directions, a live delta
  recooking the In SOP, and an answer to Nomad's *Get*
