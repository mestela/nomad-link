...
"""One long-lived Nomad Link connection per Maya session.

The socket lives in transport.Connection's own thread; everything else runs on
Maya's main thread from a timer callback, so scene building only ever touches
the decoded mesh cache -- never the network.

Shared with the Houdini bridge: transport.py, convert.py and this file are the
same in both, and should be kept in sync.
"""
import json
import os
import time
...
# honest hello: this build reads a scene into Maya and sends nothing back, so
# no selection_transfer, no scene_edits from us, and no "camera" (which would
# promise our working view).
CAPABILITIES = [
    "scene_transfer",
    "object_state",
    "session_config",
...
def texture_cache():
    """Where texture blobs land, so file nodes can point at real files."""
    try:
        import maya.cmds as cmds
        base = cmds.internalVar(userTmpDir=True)
    except Exception:
        import tempfile
...
class Client:
    ...
    def _install_pump(self):
        """Drain the queue from Maya's main thread, via a timer callback."""
        if self._callback is not None:
            return
        try:
            from maya.api import OpenMaya
        except ImportError:
            return  # headless: whoever owns the loop calls pump()
        try:
            self._callback = OpenMaya.MTimerMessage.addTimerCallback(
                self.PUMP_PERIOD, lambda *args: self.pump())
        except Exception as exc:
            self._callback = None
            self.note("could not install the timer callback: %s" % exc)
    ...
    def _remove_pump(self):
        ...
        try:
            from maya.api import OpenMaya
            OpenMaya.MMessage.removeCallback(self._callback)
        except Exception:
            pass
        self._callback = None
    ...
    def _handle(self, header, binary):
        ...
        if kind == "hello":
            ...
            self._pending_acks.pop(header.get("request_id", ""), None)
        elif kind == "session_config":
            self.session_config = header
        elif kind in ("request_mesh", "request_selection", "request_scene"):
            pass  # import only for now: nothing to send back

    @staticmethod
    def _nodes():
        """The Maya-facing module, or None when running headless."""
        try:
            from . import scene
        except ImportError:
            return None
